...
from game_actions import ACTION_COUNT, NOOP_ACTION
...
class RedGymEnv(Env):
    def __init__(self, config=None):
        ...
        self.ram_path = Path(config["ram_path"]) if config.get("ram_path") else None
        self.act_freq = config["action_freq"]
        self.max_steps = config["max_steps"]
        self.save_video = config["save_video"]
        self.fast_video = config["fast_video"]
        self.collect_stats = config.get("collect_stats", False)
        self.frame_stacks = 3
        self.explore_weight = (
        ...
        # Seven physical buttons plus an explicit NOOP. Controllers used NOOP
        # historically, but it was outside the declared Gym space and was
        # recorded as DOWN in the observation.
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "recent_actions": spaces.MultiDiscrete([ACTION_COUNT] * self.frame_stacks)
            }
        )
        ...
        #log_level("ERROR")
        # Never let multiple trainers implicitly share ``ROM.gb.ram``. PyBoy
        # accepts a file-like cartridge RAM source, so each trainer owns a
        # private 32 KiB Gen I save buffer and receives a portable .sav on
        # shutdown.
        ram_bytes = bytes(32 * 1024)
        if self.ram_path and self.ram_path.exists():
            candidate = self.ram_path.read_bytes()
            if len(candidate) != 32 * 1024:
                raise ValueError(
                    f"Save inválido: {self.ram_path} tem {len(candidate)} bytes; esperado 32768"
                )
            ram_bytes = candidate
        self.ram_buffer = io.BytesIO(ram_bytes)
        self.pyboy = PyBoy(
            str(self.gb_path),
            #debugging=False,
            #disable_input=False,
    ...
    def refresh_after_external_state_load(self):
        """Rebuild episode baselines after a subclass loads another state.

        Gym reset first loads ``init_state``. Hybrid journeys then restore the
        trainer's carried state. Without rebuilding these fields, the returned
        observation still depicts the initial save and the next reward credits
        every event that existed in the carried save as new progress.
        """
        current_frame = self.render()
        for index in range(self.frame_stacks):
            self.recent_screens[:, :, index] = current_frame[:, :, 0]
        self.recent_actions.fill(NOOP_ACTION)

        self.last_health = self.read_hp_fraction()
        self.party_size = self.read_m(0xD163)
        self.total_healing_rew = 0
        self.base_event_flags = sum(
            self.bit_count(self.read_m(address))
    ...
    def render(self, reduce_res=True):
        ...
        if reduce_res:
            # The policy only needs a 2x downsampled grayscale frame. The
            # scikit-image transform is accurate but expensive at every
            # environment step. Reshape + mean preserves the 72x80 output
            # without the transform overhead.
            height, width, channels = game_pixels_render.shape
            game_pixels_render = game_pixels_render.reshape(
    ...
    def run_action_on_emulator(self, action):
        # Handle the canonical NOOP or defensive invalid values.
        if action == NOOP_ACTION or action < 0 or action >= len(self.valid_actions):
            # Just tick without input
            render_screen = self.save_video or not self.headless
            self.pyboy.tick(self.act_freq, render_screen, False)
        ...
        render_screen = self.save_video or not self.headless
        press_step = 8
        self.pyboy.tick(press_step, render_screen, False)
        self.pyboy.send_input(self.release_actions[action])
        self.pyboy.tick(self.act_freq - press_step - 1, render_screen, False)
    ...
    def close(self):
        """Release video writers and the PyBoy instance explicitly."""
        self._close_video_writers()
        ...
        if pyboy is not None:
            try:
                if self.ram_path:
                    ...
                    exported_ram = io.BytesIO()
                    pyboy.stop(ram_file=exported_ram)
                    payload = exported_ram.getvalue()
                    temporary = self.ram_path.with_suffix(self.ram_path.suffix + ".tmp")
                    temporary.write_bytes(payload)
                    temporary.replace(self.ram_path)
                else:
    ...
    def read_event_bits(self):
        # Keep the original most-significant-bit-first ordering while
        # replacing thousands of formatted string operations with NumPy's
        # vectorized unpacking.
        event_bytes = np.fromiter(
            (self.read_m(i) for i in range(event_flags_start, event_flags_end)),
            dtype=np.uint8,
            count=event_flags_end - event_flags_start,
        )
        return np.unpackbits(event_bytes).astype(np.int8, copy=False)
