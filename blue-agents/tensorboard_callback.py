...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.training_env.env_method("check_if_done", indices=[0])[0]:
            ...
            # Fast training can disable per-step diagnostic collection. Keep
            # the callback useful for maps/rewards without indexing empty
            # histories.
            if not all(stats for stats in all_infos):
                return True
            ...
            # The old callback assumed exactly two environments. Keep the
            # image logging valid for one agent as well as larger runs.
            map_row = np.concatenate(explore_map, axis=0)
