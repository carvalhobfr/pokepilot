...
class RedGymEnv(Env):
    def __init__(self, config=None):
        ...
        self.init_state = Path(__file__).parent / "init.state"
    ...
    def reset(self, seed=None, options={}):
        ...
        # Try to load init.state from saved_states folder first, then current dir
        if os.path.exists("saved_states/init.state"):
            self.init_state = "saved_states/init.state"
        elif os.path.exists("init.state"):
            self.init_state = "init.state"
        else:
            raise FileNotFoundError("init.state not found in . or saved_states/")
    ...
    def run_action_on_emulator(self, action):
        # Handle PASS action (7) or invalid actions
    ...
    def get_current_coord_count_reward(self):
        ...
        # Check if text window is open (0xCFC4 or similar check)
        # If text is open, being "stuck" is actually listening/reading, so reduce penalty
        # Note: 0xCFC4 might not be reliable for all text, but 0xD057 (battle) is handled separately.
        # Let's use a simple heuristic: if we are in a script sequence (handled by HybridAgent), we might be still.
        
        # For now, just penalize excessive loitering unless in battle
        if self.read_m(0xD057) != 0:
            return 0
            
        return 0 if count < 300 else 1 # Reduced threshold for penalty to encourage movement
    ...
    def update_recent_actions(self, action):
        ...
        # Clamp action to valid range to prevent One-Hot error
        if action >= len(self.valid_actions):
            action = 0 # Default to first action (usually DOWN or NOOP equivalent) if invalid/PASS
    ...
    def get_game_state_reward(self, print_stats=False):
        ...
        # Curriculum Learning: Adjust reward weights based on progress
        badge_count = self.get_badges()
        party_count = self.read_m(0xD163)
        
        # Phase 1: Early Game (No Pokemon yet) - Heavy exploration focus
        if party_count == 0:
            explore_mult = 3.0  # Heavy exploration
            event_mult = 100   # Super high events (Oak Event)
        # Phase 2: Got starter, heading to Brock (0 badges) - Balanced
        elif badge_count == 0:
            explore_mult = 2.5
            event_mult = 60
        # Phase 3: Post-Brock (1+ badges) - Focus on progression
        else:
            explore_mult = 1.5
            event_mult = 50
        
        state_scores = {
            "event": self.reward_scale * self.update_max_event_rew() * event_mult,
            "heal": self.reward_scale * self.total_healing_rew * 10,
            "badge": self.reward_scale * self.get_badges() * 100,
            "explore": self.reward_scale * self.explore_weight * len(self.seen_coords) * explore_mult,
            "stuck": self.reward_scale * self.get_current_coord_count_reward() * -0.01
        }
